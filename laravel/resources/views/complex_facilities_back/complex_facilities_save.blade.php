...
<div class="card">
  <div class="card-header">
                                    <strong>综合设施修改</strong>
                                </div>                              
<div class="card-block">
    <form action="" method="post" enctype="multipart/form-data" class="form-horizontal ">
        <div class="form-group row">
         <input type="hidden" name="id" value="{{$one->complex_facilities_id}}">
            <input type="hidden" name="_token" value="{{ csrf_token() }}" />
        </div>
        <div class="form-group row">
            <label class="col-md-3 form-control-label" for="text-input">综合设施名称</label>
            <div class="col-md-9">
                <input type="text" id="text-input" name="complex_facilities_name" value="{{$one->complex_facilities_name}}"   class="form-control" >
            </div>
        </div>
